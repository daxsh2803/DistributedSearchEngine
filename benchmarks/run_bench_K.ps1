#!/usr/bin/env pwsh
# Phase 27 Benchmark K - High-Concurrency Stress Benchmark Orchestrator
#
# Launches a 3-node cluster (N=3, S=3, R=3) with max_concurrent_requests=64,
# runs benchmarks/bench_K_stress.py, and cleanly stops the cluster.
...
$BENCH_PY = Join-Path $PSScriptRoot_Or_Benchmarks "bench_K_stress.py"
if (-not (Test-Path $BENCH_PY)) {
    $BENCH_PY = "benchmarks\bench_K_stress.py"
}
...
$procs = @()
try {
    Write-Host "Starting 3-node cluster for Benchmark K (max_concurrent=$MAX_CONCURRENT)..."
    foreach ($n in $nodeList) {
    ...
    Write-Host "Cluster healthy. Running Benchmark K..."

    python $BENCH_PY --port 8081 --rpc-ports 9081 9082 9083 --requests-per-tier 1000 --output-dir $RESULTS
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Benchmark K failed with exit code $LASTEXITCODE"
    }
} finally {
