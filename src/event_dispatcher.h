...
//   rejects the message. There is no durable outbox — events not yet
//   enqueued are lost on crash.
//
// Backpressure:
...
public:
    ...
    // Create a dispatcher backed by the given broker.
    // Does NOT take ownership of the broker — the caller must ensure
    // the broker outlives the dispatcher.
...
private:
    ...
    // --- Statistics (protected by mutex_ for queue depth, atomics for counters) ---
